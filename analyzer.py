...
ПРАВИЛА (жёсткие SKIP — применяются ДО score, перевешивают всё остальное):
- Город не из списка (TLV/RG/Гиватаим/Бней-Брак/Бат-Ям/Холон) → SKIP, score 0.
- Продажа квартиры (a не аренда) → SKIP, score 0.
...
- Краткосрочная аренда < 6 месяцев / посуточно → SKIP, score 0.
- Цена > 8000 шек → SKIP, score 0.
- rooms_found < 2.5 (то есть студия, 1 или 2 комнаты) → SKIP, score 0.
  Это работает даже если все остальные критерии идеальны.
  Помни: израильский счёт — гостиная это комната. «2 комнаты» = спальня + гостиная.
- Только миклат / общее убежище в здании, без мамада в квартире → SKIP, score 0.
- Мамад вообще не упомянут в тексте → has_mamad: null, score максимум 3 → SKIP.

Если ни одно жёсткое правило не сработало — оценивай по шкале:
  SEND если score >= 7
  MAYBE если score 5-6
  SKIP если score <= 4

Балансировка для подходящих по жёстким правилам:
- Бат-Ям или Холон вместо Тель-Авива → −1 балл.
- Цена 7,500–8,000 → −1 балл.
- Все четыре главных критерия выполнены (правильный город, ≥2.5 комнат,
  цена ≤7,500, мамад в квартире) — это базовый уровень 7+.
"""
