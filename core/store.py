...
class Store:
    ...
    async def queue_for(self, profile_id: int, limit: int = 50,
        ...
        cur = await self._db.execute(
            ...
            + _freshness_sql("l") +
            # Граница считается по дате публикации, а не по моменту сбора.
            # Разница видна на первом же новом пользователе: как только он
            # укажет город, доска сканируется целиком, и сотни объявлений
            # недельной давности будут собраны сегодня. По моменту сбора все
            # они окажутся «новыми» и вывалятся ему в первый же час; по дате
            # публикации — останутся накопленным и лягут в /feed, где им и
            # место. Даты приходят в разных форматах, поэтому обе стороны
            # приводятся к общему виду.
            (" AND datetime(COALESCE(l.posted_at, l.collected_at)) > datetime(?)"
             if since else "") +
