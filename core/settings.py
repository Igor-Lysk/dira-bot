...
# Потолок расходов на модель за сутки. Регистрация открытая, города выбирает
# пользователь, и восемь городов у одного человека означают вчетверо больший
# поток объявлений через модель. Реальный расход — около $0.05 за цикл, то есть
# доллар-полтора в сутки; пятёрка это не бюджет, а предохранитель от аварии.
DAILY_LLM_BUDGET_USD = float(os.environ.get("DAILY_LLM_BUDGET_USD", "5"))
ENRICH_BATCH = 25
...
# Страницы объявлений: только для подошедших, поэтому редко и понемногу.
DETAILS_INTERVAL_MIN = 20
DETAILS_BATCH = 15
